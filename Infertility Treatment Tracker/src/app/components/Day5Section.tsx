...
import { Day5Record } from '../types';

interface Day5SectionProps {
  day5?: Day5Record;
  onUpdate: (day5: Day5Record) => void;
}

export function Day5Section({ day5, onUpdate }: Day5SectionProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isEditing, setIsEditing] = useState(!day5);
  const [formData, setFormData] = useState<Day5Record>(
    day5 || {
      totalEmbryos: 0,
      nextPlan: undefined,
  ...
  if (!isOpen && !day5) {
    return (
      <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-200">
        ...
        >
          <h2 className="text-xl text-gray-700">5일 배양 기록</h2>
      ...
      <div className="bg-white rounded-lg p-5 shadow-sm border border-gray-200">
        ...
        >
          <div className="flex items-center gap-3">
            <h2 className="text-xl">5일 배양 기록</h2>
            <span className="text-sm text-blue-600">{day5.totalEmbryos}개</span>
            {day5.nextPlan && (
              <span className="text-xs text-gray-500">· {getPlanLabel(day5.nextPlan)}</span>
            )}
          </div>
          <ChevronDown className="w-5 h-5 text-gray-400" />
        </button>
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
        ...
        <div className="space-y-2">
          <div className="flex justify-between">
            <span className="text-gray-600">5일 배아 수:</span>
            <span className="text-blue-600">{day5.totalEmbryos}개</span>
          </div>
          
          {(day5.gradeA !== undefined || day5.gradeB !== undefined || day5.gradeC !== undefined) && (
            <div className="mt-3 pt-3 border-t border-gray-200">
              <div className="text-sm text-gray-600 mb-2">등급별 수:</div>
              {day5.gradeA !== undefined && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">A등급:</span>
                  <span>{day5.gradeA}개</span>
                </div>
              )}
              {day5.gradeB !== undefined && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">B등급:</span>
                  <span>{day5.gradeB}개</span>
                </div>
              )}
              {day5.gradeC !== undefined && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">C등급:</span>
                  <span>{day5.gradeC}개</span>
                </div>
              )}
            </div>
          )}
          ...
          {day5.nextPlan && (
            <div className="mt-3 pt-3 border-t border-gray-200">
              <div className="flex justify-between mb-2">
                ...
                <span className="font-medium">{getPlanLabel(day5.nextPlan)}</span>
              </div>
              {day5.nextPlan === 'transfer' && (
                <>
                  {day5.transferCount !== undefined && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">이식 개수:</span>
                      <span>{day5.transferCount}개</span>
                    </div>
                  )}
                  {day5.frozenCount !== undefined && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">동결 개수:</span>
                      <span>{day5.frozenCount}개</span>
                    </div>
                  )}
                </>
              )}
              {day5.nextPlan === 'freeze' && day5.frozenCount !== undefined && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">동결 개수:</span>
                  <span>{day5.frozenCount}개</span>
                </div>
              )}
              {day5.nextPlan === 'pgt' && day5.pgtCount !== undefined && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">PGT 보낸 개수:</span>
                  <span>{day5.pgtCount}개</span>
                </div>
              )}
            </div>
          )}
          ...
          {day5.memo && (
            <div className="mt-3 pt-3 border-t border-gray-200">
              ...
              <p className="mt-1 text-sm">{day5.memo}</p>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm mb-1">5일 배아 수 *</label>
          <input
            type="number"
            min="0"
          ...
          <div className="flex gap-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                ...
                name="nextPlan"
                value="pgt"
                checked={formData.nextPlan === 'pgt'}
                onChange={(e) => setFormData({ 
                  ...formData, 
                  nextPlan: e.target.value as 'pgt',
                  transferCount: undefined,
                  frozenCount: undefined 
                })}
                className="w-4 h-4"
              />
              <span className="text-sm">PGT</span>
            </label>
          </div>
        </div>
