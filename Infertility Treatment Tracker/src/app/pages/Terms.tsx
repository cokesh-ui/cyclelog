...
export default function Terms() {
  ...
  const [expandRequired, setExpandRequired] = useState(false);
  ...
  const handleAgree = async () => {
    setSaving(true);
    try {
      await api.agreeTerms({ marketingEmail, marketingSms, marketingPush });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-amber-50 py-8 px-6">
      <div className="w-full max-w-sm mx-auto">
        <div className="text-center mb-6">
          <h1 className="text-xl font-bold text-gray-800">서비스 이용 동의</h1>
          <p className="text-sm text-gray-500 mt-1">원활한 서비스 이용을 위해 약관에 동의해주세요</p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-orange-100 space-y-3">
          ...
          <div>
            <button
              type="button"
              onClick={() => { toggleAllRequired(); if (!allRequired) setExpandRequired(false); }}
            ...
            >
              ...
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); setExpandRequired(!expandRequired); }}
                className="p-1"
              >
                <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandRequired ? 'rotate-180' : ''}`} />
              </button>
            </button>
          ...
          <div className="pt-3 border-t border-gray-100">
            <button
              type="button"
              onClick={() => { toggleAllMarketing(); if (!allMarketing) setExpandMarketing(false); }}
              className="flex items-center justify-between w-full"
            >
              <label className="flex items-center gap-2.5 cursor-pointer" onClick={(e) => e.stopPropagation()}>
                <input
              ...
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); setExpandMarketing(!expandMarketing); }}
                className="p-1"
              >
                <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${expandMarketing ? 'rotate-180' : ''}`} />
              </button>
            </button>
        ...
        <button
          ...
          className="w-full mt-5 px-4 py-3 bg-orange-500 text-white rounded-xl font-medium transition-colors disabled:opacity-40 hover:bg-orange-600"
        >
          {saving ? '처리중...' : '동의하고 시작하기'}
        </button>
      </div>
